...
function PRP() {
  ...
  const [PRPType, setPerimetryType] = useState('');
  const [time, setTime] = useState('');
  const [date, setDate] = useState('');
  const [doctor, setDoctor] = useState('');
  const [PRPDoctors, setPerimetryDoctors] = useState([]);
  ...
  const fetchData = async () => {
    try {
      ...
      let url = `${BASE_URL}/PRP?page=${currentPage}&limit=${limit}&serialToday=true`;
      ...
      if (!response.ok) throw new Error('Failed to fetch PRP data');
      ...
      console.error('Error fetching PRP data:', error);
      setError('Failed to load PRP data. Please try again.');
      setSubmittedData([]);
    } finally {
      setLoading(false);
    }
  ...
  const fetchTypes = async () => {
    try {
      ...
      if (!response.ok) throw new Error('Failed to fetch PRP types');
      const data = await response.json();
      setTypesData(data.data || []);
    } catch (error) {
      console.error('Error fetching PRP types:', error);
      setTypesData([]);
    }
  };
  ...
  const fetchPerimetryDoctors = async () => {
    try {
      const response = await fetch(`${BASE_URL}/PRP/PRP-doctors`, {
        credentials: 'include',
      });
      if (!response.ok) throw new Error('Failed to fetch PRP doctors');
      const data = await response.json();

      setPerimetryDoctors(data.data || []);
    } catch (error) {
      console.error('Error fetching PRP doctors:', error);
      setPerimetryDoctors([]);
    }
  };
  ...
  const handleSearchChange = async (patientId) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${BASE_URL}/PRP/search/${patientId}`, {
        method: 'GET',
        credentials: 'include',
      });
  ...
  const handleEdit = (index) => {
    ...
    setFieldValues({
      ...
      PRPType: recordToEdit.PRPType || recordToEdit.type || '',
  ...
  const handleRemove = async (index) => {
    if (!window.confirm('Are you sure you want to delete this PRP record?'))
      return;
    try {
      setLoading(true);
      const { _id } = submittedData[index];
      const response = await fetch(`${BASE_URL}/PRP/${_id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      if (!response.ok) throw new Error('Failed to delete PRP record');

      const updatedData = submittedData.filter((_, i) => i !== index);
      setSubmittedData(updatedData);
    } catch (error) {
      console.error('Error deleting PRP record:', error);
  ...
  const fields = [
    ...
    { label: 'Date', type: 'date', name: 'date' },
    {
      label: 'Doctor',
      type: 'select',
      options: Array.isArray(PRPDoctors)
        ? PRPDoctors.map((doctor) => ({
  ...
  return (
    <div className='p-4 sm:p-6 min-h-screen w-full'>
      ...
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6'>
        <h2 className='font-semibold text-xl mb-4 sm:mb-0'>PRP</h2>
        <button
          onClick={() => {
            clearForm();
            setIsOpen(true);
      ...
      <FormModal
        title={editMode ? 'Edit PRP Record' : 'PRP Record'}
        isOpen={isOpen}
        handleCancel={handleCancel}
        fields={fields}
        fieldValues={fieldValues}
        setFieldValues={setFieldValues}
        url={editMode ? `${BASE_URL}/PRP/${id}` : `${BASE_URL}/PRP/`}
      ...
      <div className='mt-10 mb-8'>
        <IncomeChart
          category="prp"
          title="PRP Service Income Analysis"
          showFilters={true}
